...
export async function createVehicle(req, res) {
  try {
    ...
    if (!licensePlate) {
      return res
        .status(400)
        .json({ message: "La patente (licensePlate) es requerida" });
    }
    if (!model) {
      return res
        .status(400)
        .json({ message: "El modelo (model) es requerido" });
    }
    if (!color) {
    ...
    if (user.vehicle) {
      return res
        .status(400)
        .json({ message: "El usuario ya posee un vehiculo" });
    }
    ...
    if (existingVehicle) {
      return res
        .status(400)
        .json({ message: "La patente (licensePlate) ya está en uso" });
    }
...
export async function getVehicleByOwnerId(req, res) {
  try {
    ...
    const vehicle = await Vehicle.findOne({ user: userId }).populate(
      "user",
      "username",
    );
