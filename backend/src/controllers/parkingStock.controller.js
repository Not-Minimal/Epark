  ...
  try {
    ...
    if (quadrantId) {
      ...
      if (!quadrant) {
        ...
        res
          .status(201)
          .json({ message: "Espacio creado correctamente", data: newSpace });
      }
    } else {
      res.status(404).json({ message: "Cuadrante no encontrado" });
    }
    ...
    console.log(
      "Error en parkingStock.controller.js -> createSpace(): ",
      error,
    );
    res.status(500).json({ message: error.message });
  }
...
export async function getAllSpaces(req, res) {
  try {
    ...
    console.log(
      "Error en parkingStock.controller.js -> getAllSpaces(): ",
      error,
    );
    res.status(500).json({ message: error.message });
  }
...
export async function getOccupiedSpacesOnQuadrant(req, res) {
  try {
    ...
    console.log(
      "Error en parkingStock.controller.js -> getOccupiedSpaces(): ",
      error,
    );
    res.status(500).json({ message: error.message });
  }
...
export async function getAvailableSpaces(req, res) {
  try {
    ...
    console.log(
      "Error en parkingStock.controller.js -> getAvailableSpaces(): ",
      error,
    );
    res.status(500).json({ message: error.message });
  }
...
export async function getAvailableSpacesOnQuadrant(req, res) {
  try {
    const quadrantId = req.params.id;
    const availableSpaces = await Space.countDocuments({
      quadrant: quadrantId,
      isOccupied: false,
    });
    res.json(availableSpaces);
...
export async function vacateSpaceOnQuadrant(req, res) {
  try {
    ...
    console.log(
      "Error en parkingStock.controller.js -> vacateSpaceOnQuadrant(): ",
      error,
    );
    res.status(500).json({ message: error.message });
  }
...
export async function deleteAllSpaces(req, res) {
  try {
    ...
    if (result.deletedCount > 0) {
      ...
      res
        .status(404)
        .json({ message: "No se encontraron espacios para eliminar" });
    }
  } catch (error) {
    console.log(
      "Error en parkingStock.controller.js -> deleteAllSpace(): ",
      error,
    );
    res.status(500).json({ message: error.message });
  }
...
export async function deleteSpaceOnQuadrant(req, res) {
  try {
    ...
    if (quadrantId) {
      ...
      if (!quadrant) {
        ...
        if (count === 0) {
          ...
          return res
            .status(404)
            .json({ message: "No hay espacios en la base de datos" });
        }
        ...
        if (!freeSpace) {
          ...
          return res
            .status(404)
            .json({ message: "No hay espacios desocupados para eliminar" });
        } else {
    ...
    console.log(
      "Error en stockEspacio.controller.js -> deleteSpaceOnQuadrant(): ",
      error,
    );
    res.status(500).json({ message: error.message });
  }
...
export async function deleteSpecifiedSpacesOnQuadrant(req, res) {
  try {
    ...
    console.log(
      "Error en stockEspacio.controller.js -> deleteSpecifiedSpacesOnQuadrant(): ",
      error,
    );
    res.status(500).json({ message: error.message });
  }
